...
(`backend/geolibre_server/geolibre_server/app/oetm.py`). Es ist als Teil des
``geolibre_server``-Packages installiert, sodass der Router es mit einem
relativen Import (``from .oetm_extractor import ...``) laden kann — ohne
``sys.path``-Manipulation und unabhängig vom Deploymentszenario (Source-Tree,
Docker, Wheel-Installation).

Die Datei ``scripts/oetm_pdf_extractor.py`` ist ein dünner Re-Export-Wrapper,
der die Standalone-Demo (``python scripts/oetm_pdf_extractor.py``) ermöglicht,
ohne das Package installieren zu müssen.

IMPLEMENTATION_PLAN ref: Phase 0.4, Anhang B.1
"""
import fitz  # PyMuPDF
